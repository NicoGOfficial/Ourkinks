...
import {
  galleryService, performerService,
...
class SearchPage extends PureComponent<IProps, any> {
  ...
  loadSearchByStats(props) {
    const {
      totalPerformers, totalVideos, totalGalleries
    } = props;
  ...
  async pageChanged(page) {
    const {
      type, performers, videos, galleries
    } = this.state;
    if (type === 'video') {
  ...
  render() {
    ...
    const {
      fetchingData, performers, videos, galleries, type, limit,
      stats
    } = this.state;
    const {
      totalGalleries = 0,
    ...
    const totalResult = totalGalleries + totalPerformers + totalVideos;
    return (
      <Layout>
        <SeoMetaHead item={{
        ...
        <div className="main-container">
          <div className={style['search-lg-top']}>
            <Input.Search placeholder={t('common:searchAnything')} enterButton defaultValue={q} onSearch={(keyword) => Router.replace({ pathname: '/search', query: { q: keyword } })} onPressEnter={(e: any) => Router.replace({ pathname: '/search', query: { q: e.target.value } })} />
          </div>
          ...
          <div className={style['page-search']}>
            ...
            >
              <TabPane
                tab={(
                  <Tooltip placement="top" title={t('common:model')}>
              ...
              {/* <TabPane
                tab={(
                  <Tooltip placement="top" title={t('common:product')}>
          ...
          {type === 'video' && !fetchingData && !videos.total && <div className="text-center">{t('common:noVideo')}</div>}
          {type === 'gallery' && !fetchingData && !galleries.total && <div className="text-center">{t('common:noGallery')}</div>}
          {/* {type === 'product' && !fetchingData && !products.total && <div className="text-center">{t('common:noProductWasFound')}</div>} */}
          {type === 'performer' && !fetchingData && !performers.total && <div className="text-center">{t('common:noResultsModel')}</div>}
