...
class GalleryViewPage extends PureComponent<IProps> {
  ...
  async getPhotos() {
    ...
    try {
      ...
      const err = await e;
      message.error(err?.message || t('common:errGettingPhoto'));
    } finally {
  ...
  handleSubscribe = async (paymentGateway = 'ccbill', type = 'monthly') => {
    ...
    if (!user._id) {
      message.error(t('common:errCheckLoginSubModel'));
      ...
      <Layout>
        ...
        <div className="main-container">
          ...
          <div className={style['photo-carousel']}>
            <div className={!isBlur ? 'list-photos' : 'list-photos blur'}>
              ...
              {!fetching && !photos.length && <p className="text-center">{t('common:noPhoto')}</p>}
          ...
          <div className="main-container">
            <Tabs activeKey={activeTab} onChange={(tab) => this.setState({ activeTab: tab })}>
              <Tabs.TabPane tab="Description" key="description">
                <p>{gallery?.description || t('common:noDescription')}</p>
              </Tabs.TabPane>
              <Tabs.TabPane tab="Comments" key="comment">
                ...
                <ListComments
                  ...
                  canReply
                />
                {comments.length < totalComments && <p className="text-center"><a aria-hidden onClick={this.moreComment.bind(this)}>{t('common:moreComment')}</a></p>}
              </Tabs.TabPane>
            </Tabs>
          </div>
        </div>
        <div className="main-container">
          <div className="related-items">
            ...
            {!relatedGalleries.requesting && !relatedGalleries.items.length && <div>{t('common:noGallery')}</div>}
          </div>
        </div>
      </Layout>
    );
...
export default connect(mapStates, mapDispatch)(withTranslation(GalleryViewPage));
