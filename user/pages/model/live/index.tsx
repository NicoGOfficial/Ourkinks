...
class PerformerLivePage extends PureComponent<IProps, IStates> {
  ...
  render() {
    const {
      loading, initialized, members, total
    } = this.state;
    ...
    return (
      <>
        <PageTitle title={t('common:goLive')} />
        <Event
          ...
          <Row>
            <Col xs={24} sm={24} md={12}>
              {canLive ? (
                ...
                <>
                  <p>{t('common:errGoLiveRequireApproval')}</p>
                  <Button
                    disabled
                    type="primary"
                    className="primary"
...
export default connect(mapStateToProps, mapDispatchs)(
  withTranslation(PerformerLivePage)
);
