...
class GalleryUpdatePage extends PureComponent<any, IStates> {
  ...
  async setCover(file) {
    ...
    try {
      ...
      message.success(t('common:updatedCoverSuccess'));
      this.getPhotosInGallery();
    } catch (error) {
  ...
  async removePhoto(file) {
    ...
    if (!window.confirm(t('common:areYouSureRemove'))) return;
    try {
      await this.setState({ submiting: true });
  ...
  render() {
    ...
    return (
      <Layout>
        <PageTitle title={t('common:editGallery')} />
        <div className="main-container">
          <PageHeader
            onBack={() => Router.back()}
            backIcon={<ArrowLeftOutlined />}
            title={t('common:editGallery')}
          />
          {!loading && gallery && (
          <FormGallery
            gallery={gallery}
...
export default withTranslation(GalleryUpdatePage);
