...
const authSagas = [
  {
    ...
    * worker(data: any) {
      try {
        ...
        if (userResp.isPerformer) Router.push(`/model/${userResp.username}`);
...

  {
    ...
    * worker(data: any) {
      try {
        ...
        const payload = _.pick(data.payload, ['name', 'username', 'password', 'gender', 'email', 'firstName', 'lastName', 'country', 'refCode']);
