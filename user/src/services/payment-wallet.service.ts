...
class PaymentWalletService extends APIRequest {
  ...
  sendTip(
    performerId: string,
    data: { amount: number; conversationId?: string, description?: string, tipWithMessage?: boolean }
  ) {
  ...
  userBuysMessagesById(id) {
    return this.post(`/payment-wallet/purchase-message/${encodeURIComponent(id)}`);
  }
}
