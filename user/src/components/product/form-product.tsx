...
class FormProduct extends PureComponent<IProps> {
  ...
  beforeUploadDigital(file) {
    ...
    if (!isLt2M) {
      message.error(t('common:errUploadProductSize'));
      return false;
    }
      ...
      >
        <Row>
          <Col md={24} xs={24}>
            <Form.Item
              name="name"
              rules={[
                { required: true, message: t('common:errNameProduct') }
              ]}
              label={t(t('common:labelName'))}
            >
              <Input placeholder={t('common:errNameProduct')} />
            </Form.Item>
          </Col>
          <Col md={12} xs={24}>
            <Form.Item name="type" label={t('common:type')} rules={[{ required: true, message: t('common:selectTypeProduct') }]}>
          ...
          <Col md={12} xs={24}>
            <Form.Item name="price" label={t('common:price')} required>
              <InputNumber style={{ width: '100%' }} min={1} />
            </Form.Item>
          </Col>
          {!isDigitalProduct && (
            <Col md={24} xs={24}>
              <Form.Item name="stock" label={t('common:stock')} required>
                <InputNumber style={{ width: '100%' }} min={1} />
              </Form.Item>
            </Col>
          )}
          ...
          <Col md={24} xs={24}>
            <Form.Item label={t('common:categories')} name="categoryIds">
          ...
          <Col md={24} xs={24}>
            <Form.Item name="description" label={t('common:labelDescription')}>
              <Input.TextArea rows={3} />
            </Form.Item>
          </Col>
          {isDigitalProduct && (
            <Col md={24} xs={24}>
              <Form.Item label={t('common:labelDigitalFile')}>
                <Upload
                  multiple={false}
                  showUploadList
                ...
                {product?.digitalFile && <div className="ant-form-item-explain" style={{ textAlign: 'left' }}><a href={product?.digitalFile} target="_blank" rel="noreferrer">{t('common:clickHereDownload')}</a></div>}
              </Form.Item>
            </Col>
          )}
          <Col md={24} xs={24}>
            <Form.Item label={t('common:lableImage')}>
        ...
        <Form.Item wrapperCol={{ ...layout.wrapperCol, offset: 4 }}>
          <Button type="primary" htmlType="submit" loading={uploading || photoUploading} disabled={uploading || photoUploading}>
            {product ? t('common:update') : t('common:upload')}
          </Button>
        </Form.Item>
      </Form>
    );
