...
class FeedForm extends PureComponent<IProps> {
    ...
    if (!addPoll) {
      ...
      this.formRef.setFieldsValue({
        pollList: [
          {
            id: 1,
            value: t('common:optionOne')
          },
          {
            id: 2,
            value: t('common:optionTwo')
          }
        ]
      });
    }
  ...
  async onsubmit(feed, values) {
    ...
    try {
      ...
      message.success((t('common:postedSuccessfully')));
  ...
  async beforeUploadThumbnail(file) {
    ...
    if (!isLt2M) {
      message.error(t('common:imageIsTooLarge5MBOrBelow'));
      return;
    }
    const reader = new FileReader();
    reader.addEventListener('load', () => {
    ...
    try {
      ...
      message.error(t('common:errorUploadThumbFile', { name: file.name }));
    } finally {
      this.setState({ uploading: false });
    }
  }
  ...
  async beforeUploadteaser(file) {
    ...
    if (!isLt2M) {
      message.error(
        t('common:imageIsTooLarge100MBOrBelow')
      );
      return;
    }
    this.teaser = file;
    try {
      ...
      message.error(t('common:errorUploadTeaserFile', { name: file.name }));
    } finally {
      this.setState({ uploading: false });
    }
  }
  ...
  async submit(payload: any) {
    ...
    if (!text.trim()) {
      message.error(t('common:plsAddDescription'));
      return;
    }
    const formValues = { ...payload };
    if (formValues.price < 1) {
      message.error(t('common:priceMustBeGreaterThan'));
      return;
    }
    if (this.teaserId) {
    ...
    if (['video', 'photo'].includes(feed?.type || type) && !fileIds.length) {
      message.error(t('common:plsAddTypeFile', { type: feed?.type || type }));
      return;
    }
    // create polls
    if (addPoll && payload.pollList?.length < 2) {
      message.error(t('common:pollsMustHaveAtLeast2Options'));
      return;
    }
    if (addPoll && payload.pollList?.length >= 2) {
      await this.setState({ uploading: true });
        ...
        >
          <div className={style['input-f-desc']}>
            <TextArea
              ...
              rows={3}
              placeholder={
                !fileIds.length ? t('common:composeNewPost') : t('common:addADescription')
              }
              allowClear
            />
                      ...
                      return _pollList?.map((arrItem) => (
                        <div style={{ marginBottom: '20px', display: 'flex' }}>
                          ...
                          <Input
                            ...
                            disabled={!!feed?._id}
                            value={arrItem.value}
                            onChange={(e) => this.formRef.setFieldsValue({
                              pollList: _pollList.map((ti) => {
                                if (ti.id === arrItem.id) {
                                  return {
                                    ...ti,
          ...
          {type !== 'text' && (
            <Form.Item>
              <Switch
                checkedChildren={t('common:ppvContent')}
                unCheckedChildren={t('common:freeContent')}
                checked={isSale}
          ...
          {isSale && (
            <Form.Item
              label={t('common:setPriceHere')}
              name="price"
              rules={[{ required: isSale, message: t('common:plsAddPrice') }]}
            >
              <InputNumber min={1} />
            </Form.Item>
          )}
          ...
          <div className={style['submit-btns']}>
            <Button
              ...
              disabled={uploading}
            >
              {!feed ? t('common:post') : t('common:updated')}
            </Button>
            {(!feed || !feed._id) && (
              <Button
                onClick={() => discard()}
...
export default withTranslation(FeedForm);
