  ...
  const getMenuChildren = () => {
    ...
    return notifications.map((notification) => ({
      ...
      label: (
        <Row
          gutter={24}
          className={`${style['notification-item']} ${!notification.read ? `${style['notification-unread']}` : `${style['notification-read']}`}`}
        ...
        >
          ...
          <Col md={3}>
            {!notification.read && (
              <ExclamationCircleOutlined title="dismiss" className={style['icon-dismiss']} />
            )}
          </Col>
