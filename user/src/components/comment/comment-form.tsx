      ...
      <div className={style['comment-form']}>
        ...
        <div className="cmt-area">
          <Form.Item
            name="content"
          >
            <TextArea disabled={!creator._id} maxLength={250} rows={!isReply ? 2 : 1} placeholder={!isReply ? t('common:addCommentHere') : t('common:addReplyhere')} />
          </Form.Item>
        ...
        <Button className={!isReply ? 'submit-btn' : ''} htmlType="submit" disabled={requesting}>
          {!isReply ? <SendOutlined /> : t('common:reply')}
        </Button>
      </div>
    </Form>
  );
