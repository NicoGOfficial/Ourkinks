        ...
        <div className={style['cmt-item']} key={item._id}>
          ...
          <div className="cmt-content">
            <div className="cmt-user">
              <span>
                <span>{item?.creator?.name || item?.creator?.username || t('common:na')}</span>
            ...
            <div className="cmt-action">
              ...
              {canReply && <a aria-hidden className={!isReply ? 'cmt-reply' : 'cmt-reply active'} onClick={() => this.setState({ isReply: !isReply })}>{t('common:reply')}</a>}
            </div>
            {isReply && (
            ...
            {canReply && totalReply > 0 && (
              <div className={style['view-cmt']}>
                <a aria-hidden onClick={this.onOpenComment.bind(this)}>
                  ...
                  {' '}
                  {!isOpenComment ? t('common:viewReply') : t('common:hideReply')}
                </a>
              </div>
            )}
          </div>
          ...
          <div className="reply-bl-list">
            <div>
              <ListComments
                ...
                canReply={false}
              />
              {comments.length < totalComments && <p className="text-center"><a aria-hidden onClick={this.moreComment.bind(this)}>{t('common:more')}</a></p>}
            </div>
          </div>
        )}
      </div>
