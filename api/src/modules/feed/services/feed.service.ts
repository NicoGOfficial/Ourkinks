...
export class FeedService {
  ...
  private async populateFeedData(feeds: FeedModel[], user: UserDto, jwtToken) {
    ...
    return feeds.map((f) => {
      ...
      if (performer) {
        ...
        feed.performer = performerDto.toPublicDetailsResponse();
      }
  ...
  public async search(req: FeedSearchRequest, user: UserDto, jwtToken) {
    ...
    const [data, total] = await Promise.all([
      this.feedModel
        .find(query)
        .sort(user?.roles?.includes('admin') ? sort : { isPinned: -1, ...sort })
  ...
  public async updateFeed(id: string, user: UserDto, payload: FeedCreatePayload): Promise<any> {
    ...
    await this.queueEventService.publish(
      new QueueEvent({
        ...
        data: {
          ...dto,
          status: payload.status || feed.status,
          oldStatus
        }
      })
    );
  ...
  public async deleteFeed(id, user) {
    ...
    if (feed.fileIds && feed.fileIds.length > 0) {
      ...
      for (const fileId of feed.fileIds) {
        await this.fileService.remove(fileId);
      }
    }
    ...
    // delete poll & vote
    await this.PollVoteModel.deleteMany({ _id: { $in: feed.pollIds } });
  ...
  public async pinToProfile(id: string, user: UserDto) {
    ...
    await this.feedModel.updateMany({ _id: { $ne: feed._id } }, { isPinned: false });
    await this.feedModel.updateOne({ _id: feed._id }, { isPinned: true, pinnedAt: new Date() });
    return true;
  }
  ...
  public async unpinToProfile(id: string, user: UserDto) {
    ...
    if (((!user.roles || !user.roles.includes('admin')) && feed.fromSourceId.toString() !== user._id.toString())) throw new ForbiddenException();
    if (!feed.isPinned) return true;
    await this.feedModel.updateOne({ _id: feed._id }, { $unset: { isPinned: false, pinnedAt: '' } });
    return true;
  }
}
