...
export class PerformerService {
  ...
  public async findOne(query) {
    return this.performerModel.findOne(query).lean();
  }
  ...
  public async create(
    ...
    if ((payload as any).dateOfBirth) {
      const age = moment().diff((payload as any).dateOfBirth, 'years');
      if (age < 18) {
  ...
  public async register(
    ...
    let inviter = null;
    // const referralCommission = await this.settingService.getKeyValue(SETTING_KEYS.INVITE_PERFORMER_COMMISSION) || 0.19;
    if (data.refCode) {
      inviter = await this.findByReferralCode(data.refCode);
      if (inviter) {
        data.invitedBy = inviter.username;
        data.invitedById = inviter._id;
        // if (inviter.startedReferralCommission) {
        //   if (inviter.stats.referrals > 5) {
        //     data.referralCommissionForInviter = inviter.startedReferralCommission - 0.05;
        //   } else {
        //     data.referralCommissionForInviter = inviter.startedReferralCommission - ((inviter.stats.referrals || 0) * 0.01);
        //   }
        // } else {
        //   data.referralCommissionForInviter = referralCommission;
        // }
      }
      delete data.refCode;
    }
    ...
    //       { _id: inviter._id },
    //       {
    //         $set: { startedReferralCommission: referralCommission }
    //       }
    //     );
    //   }
    // }
  ...
  public async findByReferralCode(code: string) {
    return this.performerModel.findOne({ referralCode: code });
  }

  public updatePaidFlatFeeForInviterStatus(_id: string | ObjectId, isPaidRewardForInviter = true) {
    return this.performerModel.updateOne({ _id }, { $set: { isPaidRewardForInviter } });
  }
}
