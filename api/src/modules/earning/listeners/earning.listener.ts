...
export class TransactionEarningListener {
  ...
  public async handleListenEarning(
    ...
    try {
      ...
      const { order, orderDetails, transaction } = event.data;
      ...
      const [
        ...
        this.settingService.getKeyValue(SETTING_KEYS.TOKEN_TIP_COMMISSION),
        ...
        if (orderDetail.sellerSource === 'performer' && orderDetail.status === ORDER_STATUS.PAID) {
          ...
          switch (orderDetail.productType) {
            ...
            case PRODUCT_TYPE.PHYSICAL_PRODUCT:
              commission = performerCommissions?.productSaleCommission || settingProductCommission;
              sourceType = 'product';
              break;
            case PRODUCT_TYPE.SALE_VIDEO:
              commission = performerCommissions?.videoSaleCommission || settingVideoCommission;
              sourceType = 'video';
              break;
            case PRODUCT_TYPE.TIP:
              commission = settingTipCommission;
              sourceType = 'tip';
              break;
            case PRODUCT_TYPE.FEED:
              commission = performerCommissions?.feedSaleCommission || settingFeedCommission;
              sourceType = 'feed';
              break;
            case PRODUCT_TYPE.YEARLY_SUBSCRIPTION:
              commission = performerCommissions?.yearlySubscriptionCommission || settingYearlyCommission;
              sourceType = 'performer';
              break;
            case PRODUCT_TYPE.MONTHLY_SUBSCRIPTION:
              commission = performerCommissions?.monthlySubscriptionCommission || settingMonthlyCommission;
              sourceType = 'performer';
              break;
            default: break;
          }
          ...
          await this.queueEventService.publish(
            new QueueEvent({
              ...
              data: newEarning.toObject()
            })
          );
        }
      }

      // check referral for inviter
      // const [user, performer] = await Promise.all([
      //   order.buyerId ? this.userService.findById(order.buyerId) : null,
      //   order.sellerId ? this.performerService.findById(order.sellerId) : null
      // ]);
      ...
      //     if (inviter) {
      //       await this.earningService.createReferralEarning(
      //         order,
      //         transaction._id,
      //         user, // buyer
      //         performer, // seller
      //         inviter, // inviter
      //         performer, // invited member
      //         'performer' // invited member role
      //       );
      //     }
      //   }
      // }
